...
const Sidebar = ({ step, className = '' }: Props) => {
      ...
      <div className="flex flex-col items-start gap-1 mb-8 px-2">
        <div className="flex items-center gap-2 px-2 py-1">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center"
            ...
            <i className="icon-es-sim-mgmt text-[18px]"
          ...
          return (
            <button
              ...
              className="
                ...
                ${!isBelow ? 'cursor-pointer' : ''}
                ${isCompleted ? 'text-brand/70' : ''}
              "
              disabled={isBelow}
              style={{ opacity: isBelow ? 0.5 : 1 }}
            >
              <i className="text-[16px] flex-shrink-0 transition-colors"
                style={{ color: isActive ? 'var(--color-brand)' : isCompleted ? 'var(--color-brand)' : 'var(--color-slate)' }}
                className={ICONS[s]} />
              <span className="truncate">{TITLES[s]}</span>
              {isCompleted && (
                <i className="icon-es-check ml-auto text-[14px]"
                  style={{ color: 'var(--color-brand)' }} />
              )}
            </button>
          )
